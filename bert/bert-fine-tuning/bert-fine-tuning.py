...
class BertForSequenceClassification:
    ...
    def forward(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        classifer_w = np.asarray(self.classifier, dtype=np.float64)
        logits = cls_hidden @ classifer_w
...
class BertForTokenClassification:
    ...
    def forward(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        classifier_w = np.asarray(self.classifier, dtype=np.float64)
        logits = hidden @ classifier_w
